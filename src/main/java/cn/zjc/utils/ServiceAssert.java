 ...
 * @function 服务层异常断言，不满足断言会抛出ServiceException
 */
public abstract class ServiceAssert {

    public ServiceAssert() {
        super();
    }
    ...
    public static void isTrue(final boolean expression, final String message, final Double value) {
        if (!expression) {
            throw new ServiceException(String.format(message, value));
        }
    }
    ...
    public static void isTrue(final boolean expression) {
        if (!expression) {
            throw new ServiceException(DEFAULT_IS_TRUE_ASSERT_MESSAGE);
        }
    }
    ...
    public static <T> T notNull(final T object) {
        if (object == null) {
            throw new ServiceException(DEFAULT_IS_NULL_ASSERT_MESSAGE);
        }
        return object;
    }
